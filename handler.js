  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'remove':
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Halloo @user, Welcome To The Groupp!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
                (chat.sBye || this.bye || conn.bye || 'Dadah @user, Semoga Tenang Di Alam Sana:v')).replace('@user', '@' + user.split('@')[0])
        ...
        break
      case 'promote':
        text = (chat.sPromote || this.spromote || conn.spromote || '@user ```Sekarang Jadi Atemin```')
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || 'Aokwoqqko, @user ```Ga Di Jadiin Atemin Lagi```')
